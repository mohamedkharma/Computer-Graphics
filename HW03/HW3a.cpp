...
HW3a::initVertexBuffer()
{
	...
	// flag for glGenBuffer
	static bool flag = 1;

	if(flag){
		// generate buffers for vertex coords and texture coords
...
HW3a::divideTriangle(vec2 a, vec2 b, vec2 c, int count)
{
	...
	{
		vec2 ab, ac, bc;
		ab = vec2((a[0] + b[0]) / 2.0, (a[1] + b[1]) / 2.0); // midpoint of ab
		ac = vec2((a[0] + c[0]) / 2.0, (a[1] + c[1]) / 2.0); // midpoint of ac
		bc = vec2((b[0] + c[0]) / 2.0, (b[1] + c[1]) / 2.0); // midpoint of bc
