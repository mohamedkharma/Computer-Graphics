...
//
void HW3b::resizeGL(int w, int h)
{
	...
	m_projection.perspective(45.0f, ar, 0.1f, 1000.0f);
}
...
void HW3b::resetMesh()
{
	...
	for (int i = 0; i < m_grid; ++i)
	{
		for (int j = 0; j < m_grid; ++j)
		{
			...
			case DIAGONALWALL:
				// PUT YOUR CODE HERE
				vec.setZ((i + j == m_grid - 1 || i + j == m_grid - 2) ? 0.3f : 0.0f);
				break;
			case SIDEWALL:
				// PUT YOUR CODE HERE
			...
			case DIAGONALBLOCK:
				// PUT YOUR CODE HERE
				vec.setZ((i + j >= m_grid - 2) ? 0.5f : 0.0f);
				break;
			case MIDDLEBLOCK:
				// PUT YOUR CODE HERE
				vec.setZ(((i > m_grid / 3 && j > m_grid / 3) && (i < m_grid * 2 / 3 && j < m_grid * 2 / 3)) ? 0.5f : 0.0f);
			...
			case CORNERBLOCK:
				// PUT YOUR CODE HERE
				vec.setZ((i > (m_grid * 3 / 4) && j > (m_grid * 3 / 4)) ? 0.4f : 0.0f);
				break;
			case HILL:
				// PUT YOUR CODE HERE
				vec.setZ((sin(PII * (float)i / (float(m_grid))) + sin(PII * (float)j / (float)m_grid)) / 4);
				break;
			case HILLFOUR:
				// PUT YOUR CODE HERE
				vec.setZ((sin(2 * PII * (float)i / (float(m_grid))) + sin(2 * PII * (float)j / (float)m_grid)) / 4);
				break;
			}
		}
	}
...
// Change display mode.
//
void HW3b::changeDisplay(int val)
{
...
//
void HW3b::wheelEvent(QWheelEvent *event)
{
